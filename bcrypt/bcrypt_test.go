...
func TestGenerateFromPasswordAndSalt(t *testing.T) {
	...
	if !bytes.Equal(actual, expected) {
		t.Errorf("GenerateFromPasswordAndSalt: expected hash %+v, got %+v", expected, actual)
	}
}
...
func TestSalt(t *testing.T) {
	hash := []byte("$2a$10$XajjQvNhvvRt5GSeFk1xFe5l47dONXg781AmZtd869sO8zfsHuw7C")
	expected := []byte{88, 97, 106, 106, 81, 118, 78, 104, 118, 118, 82, 116, 53, 71, 83, 101, 70, 107, 49, 120, 70, 101}
	actual, err := Salt(hash)
	if err != nil {
		t.Errorf("Salt, error: %s", err)
		return
	}
	if !bytes.Equal(actual, expected) {
		t.Errorf("Salt, expected %+v, got: %+v", expected, actual)
	}
}
