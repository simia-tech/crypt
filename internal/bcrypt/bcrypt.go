...
// GenerateFromPasswordAndSalt works like GenerateFromPassword, but uses
// the provided salt instead of random one.
...
// Salt returns the salt of the provided hashed password.
func Salt(hashedPassword []byte) ([]byte, error) {
	p, err := newFromHash(hashedPassword)
	if err != nil {
		return nil, err
	}
